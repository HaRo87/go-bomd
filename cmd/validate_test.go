...
func TestValidateTemplateCmdInvalidFileReturnsError(t *testing.T) {
	hook := test.NewGlobal()
	err := executeCommand(rootCmd, "validate", "template", "-vvv", "-f", "some.tmpl")
	assert.Error(t, err)
	assert.Equal(t, 2, len(hook.Entries))
	assert.Equal(t, logrus.ErrorLevel, hook.LastEntry().Level)
	assert.Equal(t, "😱 something went wrong", hook.LastEntry().Message)
...
func TestValidateTemplateCmdSuccess(t *testing.T) {
	hook := test.NewGlobal()
	assert.NoError(t, validateBomCmd.Flags().Lookup("licenseCheck").Value.Set("false"))
	err := executeCommand(rootCmd, "validate", "template", "-vvv", "-f", "../examples/templates/bom.tmpl")
	assert.NoError(t, err)
	assert.Equal(t, 2, len(hook.Entries))
	assert.Equal(t, logrus.InfoLevel, hook.LastEntry().Level)
	assert.Equal(t, "😎 everything seems to be fine", hook.LastEntry().Message)
